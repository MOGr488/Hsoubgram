<x-app-layout>
    ...
    <div class="grid grid-cols-4">
        ...
        <div class="px-4 col-span-2 md:ml-0 flex flex-col md:flex-row order-2 md:col-span-3">
            <div class="text-3xl mb-3 ltr:mr-3 rtl:ml-3 dark:text-white">{{ $user->username }}</div>
            @auth
                @if ($user->id === auth()->id())
                    <a href="/{{ $user->username }}/edit"
                        class="w-32 h-8 self-start border text-sm font-bold py-1 px-1 rounded-md border-neutral-300 text-center dark:text-white">
                        {{ __('Edit Profile') }}
                    </a>
                @else
                    <livewire:follow-button :userId="$user->id" classes="bg-blue-400 text-white"
                        wire:click="$emit('toggleFollow')" />
                @endif
        ...
        {{-- User Bio --}}
        <div class="text-md mt-8 px-4 col-span-3 col-start-1 order-3 md:col-start-2 md:order-4 md:mt-0 dark:text-white">
        ...
        <div
            ...
            <ul class="text-md flex flex-row justify-around md:justify-start md:space-x-10 md:text-xl">
                <li class="flex flex-col md:flex-row text-center rtl:ml-5 dark:text-white">
                    <div class="md:ltr:mr-1 md:rtl:ml-1 font-bold md:font-normal">
                        {{ $user->posts->count() }}
                    </div>
                    <span class='text-neutral-500 dark:text-white'>
